...
@
The output lists all variables stored in the GDS file. At the first level, it saves variables \vr{sample.id}, \vr{snp.id}, etc. The second-level variables \vr{sex} and \vr{pop.group} are both stored in the directory of \vr{sample.annot}. All of the functions in \hn{SNPRelate} require a minimum set of variables in the SNP annotation data.  The minimum required variables are
\begin{itemize}
...
\begin{itemize}
	\item{\vr{snp.rs.id}, a character string identifier for the SNP that may not be unique.}
...
\end{itemize}
There are four possible values stored in the variable \vr{genotype}: 0, 1, 2 and 3. ``0'' indicates two B alleles, ``1'' indicates one A allele and one B allele, ``2'' indicates two A alleles, and ``3'' is a missing genotype. ``Bit2'' indicates that each byte encodes up to four SNP genotypes since one byte consists of eight bits.
<<>>=
...
% -------- Format conversion from Sequence VCF files --------
\subsection{Format conversion from Sequence VCF files}

The \hn{SNPRelate} package provides a function \fn{snpgdsVCF2GDS} for converting a VCF file to a GDS file:
...
\subsubsection{PLINK method of moments (MoM)}
<<>>=
...
Funding: National Institutes of Health (HG 004446).
