...
challenges. We developed \Biocpkg{gdsfmt} and \Biocpkg{SNPRelate}
...
\Biocpkg{gdsfmt} to provide efficient, platform independent memory
...
% ---- Installation ----
\section{Installation of the package SNPRelate}

To install the package \Biocpkg{SNPRelate}, you need a current version
($>=2.14.0$) of \R{} (\url{www.r-project.org}) and the R package
\Biocpkg{gdsfmt}. After installing \R{} you can run the following commands
from the \R{} command shell to install the R package \Biocpkg{SNPRelate}.
...
\Biocpkg{gdsfmt} package provides the genomic data structure (GDS)
...
efficient random access to large data sets. A tutorial for the R/Bioconductor
package \Biocpkg{gdsfmt} can be found:
\url{http://corearray.sourceforge.net/tutorials/gdsfmt/}.
...
\Rfunction{index.gdsn} are defined in the package \Biocpkg{gdsfmt}:
...
We developed \Biocpkg{gdsfmt} and \Biocpkg{SNPRelate}
(high-performance computing R packages for multi-core symmetric multiprocessing
computer architectures) to accelerate two key computations in GWAS: principal
component analysis (PCA) and relatedness analysis using identity-by-descent
(IBD) measures.
...
<<PCA, fig.width=5, fig.height=5, fig.align='center'>>=
# make a data.frame
...
<<PCA-Color, fig.width=5, fig.height=5, fig.align='center'>>=
# Make a data.frame
tab <- data.frame(sample.id = pca$sample.id,
...
<<PCA-Pair, fig.width=5, fig.height=5, fig.align='center'>>=
...
<<PCA-CORR, fig.width=5, fig.height=5, fig.align='center'>>=
...
% -------- Fst Estimation --------
\subsection{$F_{st}$ Estimation}

Given two or more populations, $F_{st}$ can be estimated by the method of
Weir \& Cockerham (1984).
...
<<IBD-MoM, fig.width=5, fig.height=5, fig.align='center'>>=
...
<<IBD-MLE, fig.width=5, fig.height=5, fig.align='center'>>=
plot(ibd.coeff$k0, ibd.coeff$k1, xlim=c(0,1), ylim=c(0,1),
...
<<IBD-KING, fig.width=5, fig.height=5, fig.align='center'>>=
...
<<IBS, fig.width=5, fig.height=5, fig.align='center'>>=
library(lattice)
...
<<IBS-MDS, fig.width=5, fig.height=5, fig.align='center'>>=
...
<<IBS-Tree, fig.width=5, fig.height=5, fig.align='center'>>=
set.seed(100)
...
<<IBS-Dendrogram, fig.width=5, fig.height=5, fig.align='center'>>=
...
testing the \Biocpkg{gdsfmt} and \Biocpkg{SNPRelate} packages.
