...
GDS is also used by an R/Bioconductor package \hn{GWASTools} as one of its data storage formats (Gogarten et al., 2012) \footnote{Stephanie M. Gogarten., {\it et al}. GWASTools: an R/Bioconductor package for quality control and analysis of Genome-Wide Association Studies. {\it Accepted}.}. \hn{GWASTools} provides many functions for quality control and analysis of GWAS, including statistics by SNP or scan, batch quality, chromosome anomalies, association tests, etc.
...
The \hn{SNPRelate} package provides a function \fn{snpgdsVCF2GDS} to extract bi-allelic and polymorhpic SNPs from a VCF file:
...
Here is the population information we known:
<<fig=TRUE, pdf=FALSE, png=TRUE, resolution=100>>=
rv2 <- snpgdsCutTree(ibs.hc, samp.group=as.factor(pop))
